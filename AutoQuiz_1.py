    ...
    for parts in listQuestionForm:
        ...
        if(questionClassType == "rc-TextInputBox"):
            ...
            for divTag in rc_group_answer:
                ...
                if(check_string_in_list(answerDB,answer)):
                    InputTag.click()
    tab.ele('tag=input@@id=agreement-checkbox-base@@aria-labelledby=check-agree').click()
    tab.wait.ele_loaded('@@data-test=submit-button@!disable=')
    tab.ele('tag=button@@type=button@@data-test=submit-button').click()
    tab.wait(3)
